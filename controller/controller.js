...
export const getBloodCellsDescriptionByNameHandler = async (request) => {
  try {
    const description = await bloodCellModel.getBloodCellDescriptionByName(firstCharToUpperCase(request.params.name));
    return description;
  } catch (err) {
    throw err;
  }
...
// allcells
export const getAllBloodCellsNamesHandler = async (request, response) => {
  try {
    const names = await bloodCellModel.getAllBloodCellsNames();
